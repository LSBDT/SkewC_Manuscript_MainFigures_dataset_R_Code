library(Rmisc)
library(ggplot2)
tgcMeta3 <- subset(tgcMeta_2,Coverage_annotation =="Good_cells")
tgc <- summarySE(tgcMeta3, measurevar="area", groupvars=c("cell_cycle_phase"))
tgc$cell_cycle_phase <- factor(tgc$cell_cycle_phase,levels = tgc$cell_cycle_phase[order(tgc$area)])
pd <- position_dodge(0.5) # move them .05 to the left and right
pdf("image_analysis_cell_cycle_analysis/Good_cells_area_cell_cycle_phase.pdf")
...
geom_point(position=pd, size=4, shape=21, fill="white",stroke = 2) + # 21 is filled circle
  ...
  ggtitle(label = "GSE46980:Good cell size & cell cycle(n=39)",subtitle = "bars representing SEM") +
...
ggplot(data = GSE46980_R1_Metadata_V5,aes(x=cell_cycle_phase,fill=image_annotation))+
  geom_bar(stat = "count")

ggplot(data = GSE46980_R1_Metadata_V5,aes(x=cell_cycle_phase,fill=Coverage_annotation))+
  geom_bar(stat = "count")


ggplot(data = tgcMeta,aes(x=Coverage_annotation,fill=cell_cycle_phase))+
  geom_bar(stat = "count")
