library(ggplot2)
tsne_plot <- data.frame(x = tsne_out$Y[,1], y = tsne_out$Y[,2], col = mm10_EMTAB_2600_metadata_processd_2$covGroup)
p1 <- ggplot(data = tsne_plot, aes(x=x, y=y, color=col)) + geom_point(size=2.0) +
  labs(title="E-MTAB-2600:Non skewed vs. Skewed single-cells", x="tSNE1", y="tSNE2") + scale_color_manual(labels = c("Good", "Skewed"), values = c("green3", "magenta")) + theme_bw() + guides(color=guide_legend("Sample type"))
...
pdf("./T-SNE_Good_Skewed_covergae/Rsne_good_skewed2.pdf")
p1
dev.off()
p1
