library(ggplot2)
tsne_plot <- data.frame(x = tsne_out$Y[,1], y = tsne_out$Y[,2], col = E_MTAB_2600_columnAnnotation_2$covGroup)
p1 <- ggplot(data = tsne_plot, aes(x=x, y=y, color=col)) + geom_point(size=2.0) +
  labs(title="E-MTAB-2600:Good vs. Skewed single-cells", x="tSNE1", y="tSNE2") + scale_color_manual(labels = c("Good", "Skewed"), values = c("blue", "red")) + theme_bw() + guides(color=guide_legend("Sample type"))
...
png("./T-SNE_Good_Skewed_covergae/Rsne_good_skewed.png",width = 1250,height=1000,res=200)
p1
dev.off()
p1
